...
const FORMS = ['粒狀','粉狀','散粉'];          // 性狀
const WEIGHTS = ['1','20','25','500','1000']; // 規格公斤（粒狀固定20，粉狀/散粉多規格，含小包1kg）
...
function renderProspects(){
  ...
  const inStatus = p => {
    ...
    return statusOf(p)===pFilter.status;
  };
  // 有機屬性筆數（依目前狀態動態）
...
function smartProspectSearch(){
  const regs=regionsSorted();
  const allBar=(which)=>`<div style="display:flex;gap:8px;margin:6px 0 8px"><button type="button" class="btn btn-out" style="padding:3px 14px;font-size:12px" onclick="ssSelAll('${which}',true)">全選</button><button type="button" class="btn btn-out" style="padding:3px 14px;font-size:12px" onclick="ssSelAll('${which}',false)">清除</button></div>`;
  let h=`<div class="info">勾選條件後按「🤖 一鍵自動爬取」，系統會自動向 Google 新聞抓取相關報導、直接帶到下方欄位，<b>不用手動貼文字</b>。只查公開資訊、不會上傳你的客戶資料。若網路或免費代理不穩抓不到，可改用「🔎 產生搜尋連結」自己找，或把作物＋縣市告訴你的 LINE 助理代爬。</div>`;
  h+=`<div class="sec-title"><span class="bar"></span>① 勾選條件（可複選 / 全選）</div><div class="card">`;
  h+=`<div class="field"><label>作物 / 農產品（可複選）</label><div class="ss-dd-head" onclick="ssDD(this)" style="display:flex;align-items:center;justify-content:space-between;border:1px solid var(--line);border-radius:10px;padding:11px 14px;background:var(--card2,#f1efe6);cursor:pointer"><span>點此挑選作物 <b id="ss-cnt-crop" style="color:#4d5d2a"></b></span><span class="ss-dd-arrow">▾</span></div><div class="ss-dd-body" style="display:none;border:1px solid var(--line);border-top:none;border-radius:0 0 10px 10px;padding:4px 12px 12px;margin-top:-1px">${allBar('crop')}${Object.keys(SS_CROP_GROUPS).map(g=>`<div style="margin-top:8px"><div style="font-size:12px;color:var(--muted,#8a8f7a);margin-bottom:3px">${g}</div><div style="display:flex;flex-wrap:wrap;gap:7px">${SS_CROP_GROUPS[g].map(c=>`<label style="display:inline-flex;align-items:center;gap:5px;white-space:nowrap;flex:0 0 auto;background:#fff;border:1px solid var(--line);border-radius:16px;padding:5px 12px;font-size:13px;line-height:1"><input type="checkbox" class="ss-cropck" value="${esc(c)}" onchange="ssToggleElev();ssCnt('crop')" style="margin:0;width:15px;height:15px;flex:0 0 auto">${esc(c)}</label>`).join('')}</div></div>`).join('')}</div><input id="ss-crop" placeholder="其他自訂（可用、分隔，例：愛文芒果）" oninput="ssToggleElev()" style="margin-top:8px"></div>`;
  h+=`<div class="field" id="ss-elev-wrap" style="display:none"><label>🫖 茶葉海拔分級</label><select id="ss-elev"><option value="">不限海拔</option><option value="高山茶">高山茶（1000m 以上）</option><option value="中海拔茶">中海拔（500–1000m）</option><option value="平地茶">低海拔 / 平地（500m 以下）</option></select></div>`;
  h+=`<div class="field"><label>區域（縣市，可複選，全不選＝全台）</label><div class="ss-dd-head" onclick="ssDD(this)" style="display:flex;align-items:center;justify-content:space-between;border:1px solid var(--line);border-radius:10px;padding:11px 14px;background:var(--card2,#f1efe6);cursor:pointer"><span>點此挑選區域 <b id="ss-cnt-reg" style="color:#4d5d2a"></b></span><span class="ss-dd-arrow">▾</span></div><div class="ss-dd-body" style="display:none;border:1px solid var(--line);border-top:none;border-radius:0 0 10px 10px;padding:10px 12px;margin-top:-1px">${allBar('reg')}<div id="ss-reg" style="display:flex;flex-wrap:wrap;gap:8px">${regs.map(r=>`<label style="display:inline-flex;align-items:center;gap:5px;white-space:nowrap;flex:0 0 auto;background:#fff;border:1px solid var(--line);border-radius:16px;padding:6px 13px;font-size:14px;line-height:1"><input type="checkbox" class="ss-regck" value="${esc(r)}" onchange="ssCnt('reg')" style="margin:0;width:16px;height:16px;flex:0 0 auto">${esc(r)}</label>`).join('')}</div></div></div>`;
  h+=`<div class="field"><label>聲量關鍵字（可複選）</label><div class="ss-dd-head" onclick="ssDD(this)" style="display:flex;align-items:center;justify-content:space-between;border:1px solid var(--line);border-radius:10px;padding:11px 14px;background:var(--card2,#f1efe6);cursor:pointer"><span>點此挑選關鍵字 <b id="ss-cnt-kw" style="color:#4d5d2a"></b></span><span class="ss-dd-arrow">▾</span></div><div class="ss-dd-body" style="display:none;border:1px solid var(--line);border-top:none;border-radius:0 0 10px 10px;padding:10px 12px;margin-top:-1px">${allBar('kw')}<div id="ss-kw" style="display:flex;flex-wrap:wrap;gap:8px">${['爆紅','網紅','直播','得獎','有機','青農','產銷履歷','友善耕作','故事'].map(k=>`<label style="display:inline-flex;align-items:center;gap:5px;white-space:nowrap;flex:0 0 auto;background:#fff;border:1px solid var(--line);border-radius:16px;padding:6px 13px;font-size:14px;line-height:1"><input type="checkbox" value="${k}" onchange="ssCnt('kw')" style="margin:0;width:16px;height:16px;flex:0 0 auto">${k}</label>`).join('')}</div></div></div>`;
...
function ssCnt(which){
  ...
  if(which==='reg'){ const n=document.querySelectorAll('.ss-regck:checked').length; const e=$('#ss-cnt-reg'); if(e)e.textContent=n?('已選 '+n+' 縣市'):''; }
}
function ssSelAll(which,on){
  const sel = which==='crop' ? '.ss-cropck' : which==='kw' ? '#ss-kw input[type=checkbox]' : '.ss-regck';
  document.querySelectorAll(sel).forEach(c=>{ c.checked=on; });
  if(which==='crop') ssToggleElev();
  ssCnt(which);
}
...
function ssElev(){
  const w=$('#ss-elev-wrap'); if(!w||w.style.display==='none') return '';
  return ($('#ss-elev')&&$('#ss-elev').value)||'';
}
function ssBuildLinks(){
  const crops=ssCrops();
  const crop=crops.join(' ');
  const elev=ssElev();
  const region=ssRegionsSel().join(' ');
  ...
  const news=encodeURIComponent([crop,elev,region,...kws,'農民'].filter(Boolean).join(' '));
  const gen=encodeURIComponent([crop,elev,region,...kws,'農場'].filter(Boolean).join(' '));
  const fb=encodeURIComponent([crop,elev,region,'農場'].filter(Boolean).join(' '));
  const yt=encodeURIComponent([crop,elev,region,'農民'].filter(Boolean).join(' '));
  const map=encodeURIComponent([crop,elev,region,'農場 茶園 果園'].filter(Boolean).join(' '));
  const links=[
...
async function ssAutoFetch(){
  ...
  const kws=[...document.querySelectorAll('#ss-kw input:checked')].map(c=>c.value);
  if(!crops.length && !ssRegionsSel().length && !kws.length){ toast('至少勾一個作物或選關鍵字'); return; }
  const box=$('#ss-auto');
  ...
  window._ssCand=[]; const seen={}; let got=0;
  for(const rg of regList){
    const cropTerms=elev?crops.filter(c=>!/茶/.test(c)):crops;
    const q=[cropTerms.join(' '),elev,rg,...kws].filter(Boolean).join(' ');
    const rss='https://news.google.com/rss/search?q='+encodeURIComponent(q)+'&hl=zh-TW&gl=TW&ceid=TW:zh-Hant';
    const xml=await fetchRss(rss); if(!xml) continue; got++;
    ...
    items.forEach(it=>{
      ...
      const title=gt('title'); if(!title||seen[title]) return; seen[title]=1;
...
// 把目標客戶標記為「追蹤中」存入管理（可於名單狀態「🎯 追蹤中」檢視）
function toggleProspectTracking(id){
  const o=overlay[id]||{};
  o.tracking=!o.tracking;
  if(o.tracking) o.trackedAt=todayStr(); else delete o.trackedAt;
...
function convertToCustomer(id){
  ...
  const c = {
    ...
    idno:'', birth:'', taxid:'', terms:'', checkPeriod:'', invoiceType:'', truck:'', deliveryTime:'',
...
function custSection(id,key){
  ...
  if(key==='basic'){
    ...
    h+=`<div class="card">`+checkPeriodHTML(c.checkPeriod||'')+invoiceTypeHTML(c.invoiceType||'');
...
function custGroup(id,key){
  ...
  if(key==='profile'){
    ...
    h+=invoiceTypeHTML(c.invoiceType||'');
...
function saveCustProfile(id){
  ...
  Object.assign(c,{ name, type, org:g('f-org'), sysno:g('f-sysno'), phone:g('f-phone'), contact:g('f-contact'),
    ...
    checkPeriod:readCheckPeriod(), invoiceType:g('f-invoiceType'), creditLimit:g('f-creditLimit'), conditions:g('f-conditions'),
...
function buildFullProfile(c){
  ...
  addd('票期',c.checkPeriod); addd('發票形式',c.invoiceType); addd('額度（萬元）',c.creditLimit);
...
function saveCustDeal(id){ const c=findCust(id); c.checkPeriod=readCheckPeriod(); const iv=$('#f-invoiceType'); if(iv)c.invoiceType=iv.value; const cr=$('#c-credit'); c.creditLimit=cr?cr.value.trim():(c.creditLimit||''); const regs=readRegionChips(); if(c.type==='經銷商') c.salesRegions=regs; else if(c.type==='直接農民') c.fertRegions=regs; c.mapLocations=readMapLocs(); saveCust(); toast('已儲存交易資料'); custSection(id,'deal'); render(); }
...
function invoiceTypeHTML(v){
  v=v||'';
  return `<div class="field"><label>發票形式</label><select id="f-invoiceType">
    <option value="" ${!v?'selected':''}>未設定</option>
    <option value="二聯式" ${v==='二聯式'?'selected':''}>二聯式</option>
    <option value="三聯式" ${v==='三聯式'?'selected':''}>三聯式</option>
  </select></div>`;
}
...
function saveCustomer(id, isAdd){
  ...
  Object.assign(base,{ id, name, type:$('#f-type').value, org:g('f-org'), sysno:g('f-sysno'), phone:g('f-phone'), contact:g('f-contact'),
    ...
    regAddress:g('f-regAddress'), checkPeriod:readCheckPeriod(), invoiceType:g('f-invoiceType'), conditions:g('f-conditions'),
    mortgageSet:($('#f-mortset')?$('#f-mortset').value:''), mortgageType:($('#f-morttype')?$('#f-morttype').value:''), mortgageAmount:g('f-mortamt'),
...
// ========== 整合路線頁：每週排程為主軸＋智慧時間安排＋加料＋單選每日＋寫入存檔 ==========
// 一頁搞定：自動收集到期/逾期/分級對象→分配各出訪日→每日就近排序與抵達時刻；
// 需要時用「加料」(條件組合/關鍵字)加入特定對象；按💾寫入存檔後，每天打開頁首即見今日行程，
...
function renderRoute(){
  ...
  // ── 當日規劃設定 ──
  h+=`<div class="sec-title" style="margin-top:6px"><span class="bar"></span>當日路線規劃</div>`;
  h+=`<div class="card">`;
  h+=`<div class="field"><label>拜訪日期</label>${rocDateInput('w-date',weekCfg.date)}</div>`;
  h+=`<div class="field"><label>最多家數</label><input type="number" id="w-max" min="1" max="12" value="${weekCfg.maxPerDay}"></div>`;
  h+=weekDDOne('mode','_ddMode','排入對象',[{v:'all',t:'到期＋依分級建議（推薦）'},{v:'due',t:'只排已設下次拜訪日'},{v:'none',t:'只排我手動挑選的'}]);
  // 篩選：點開打勾清單下拉複選（不選＝全部）
  ...
  h+=renderLocCard();
  // ── 產生當日路線 ──
  h+=`<div class="btn-row"><button class="btn btn-pri" onclick="planWeek()">📅 產生當日路線</button></div>`;
...
function renderAddOn(){
  ...
  if(!weekCfg._addOpen){
    h+=`<div class="card" style="padding:12px 14px;color:var(--muted);font-size:12.5px">把「自動收集」以外的特定對象也排進來：用關鍵字挑單一客戶/名單，或用條件組合（例如 陌生農會1）整批加入。${(smartCfg.picks.length||comboN)?`<br>目前已加：指定 ${smartCfg.picks.length} 家＋條件 ${comboN} 組。`:''}</div>`;
    return h;
  }
  // 條件組合
  ...
  if(smartCfg.picks.length){
    ...
    smartCfg.picks.forEach((p,i)=>{ h+=`<div class="pk-row" data-k="${p.key}" style="padding:8px 0;border-bottom:1px solid var(--line);display:flex;align-items:center;gap:8px"><div class="avatar" style="background:${colorFor(p.name)};width:26px;height:26px;font-size:13px">${i+1}</div><div style="flex:1;min-width:0"><div class="nm" style="font-size:14px">${esc(p.name)}</div><div class="sub" style="font-size:11.5px">${esc([p.district,p.channel].filter(Boolean).join(' · '))}</div></div><button class="rule-del" onclick="smartRemovePick('${p.key}')">✕</button></div>`; });
    h+=`</div>`;
  }
  // ② 進階：條件組合整批加入
  h+=`<div class="card"><div class="field"><label>🤖 進階：條件組合整批加入（自動拆解，挑到期優先）</label><input id="r-nl" value="${esc(routeCfg.nl)}" placeholder="例如：陌生農會1 既有合作社2"></div>
    <div class="btn-row" style="margin-top:0"><button class="btn btn-out" onclick="parseNL()">✨ 解析成組合</button></div>`;
...
function renderExtras(){
  ...
  h+=`<div class="field-2"><div class="field"><label>日期</label>${rocDateInput('ex-date', weekCfg.date)}</div><div class="field"><label>時間</label><input type="time" id="ex-time" value="10:00"></div></div>`;
  ...
  h+=`<div class="tagline" style="margin-top:6px">約訪日期需與上方「拜訪日期」相同，才會排進當日路線（鎖定該時段，其餘行程就近銜接）。</div>`;
  h+=`</div>`;
  return h;
}
function addExtra(){
...
function scheduleDayPlan(items, cfg, opts){
  ...
  const s=toMin(cfg.startTime||'08:00');
  ...
  const insertLunchBefore=(stopAddr)=>{
    ...
    if(naive < ls) return;
    let larr = lunchLoc ? t + (hasPrev()?smartTravel(prevAddr,lunchLoc):0) : t; larr=Math.max(larr,ls);
    result.push({lunch:true,arrive:fmt(larr),leave:fmt(larr+lunchDur),address:lunchLoc||''});
    t=larr+lunchDur; if(lunchLoc) prevAddr=lunchLoc; lunchDone=true;
  };
  let fi=0, ui=0;
  while(ui<free.length || fi<fixed.length){
    const nextFx = fi<fixed.length ? fixed[fi] : null; let useFixed=false;
    if(nextFx){
      if(ui>=free.length) useFixed=true;
      else if(nextFx._at <= t + smartTravel(prevAddr, nextFx.address)) useFixed=true;
      else { const fr=free[ui]; const afterFree = t + (hasPrev()?smartTravel(prevAddr,fr.address):0) + dwellOf(fr) + smartTravel(fr.address, nextFx.address); if(afterFree > nextFx._at) useFixed=true; }
    }
    if(useFixed){ const fx=fixed[fi++]; insertLunchBefore(fx.address); let arr=Math.max(t + (hasPrev()?smartTravel(prevAddr,fx.address):0), fx._at); if(arr>fx._at) lateFix=true; result.push(Object.assign({},fx,{arrive:fmt(arr),leave:fmt(arr+dwellOf(fx)),_dur:dwellOf(fx),fixedMark:true})); t=arr+dwellOf(fx); prevAddr=fx.address; }
    else{ const p=free[ui++]; insertLunchBefore(p.address); let arr=t + (hasPrev()?smartTravel(prevAddr,p.address):0); result.push(Object.assign({},p,{arrive:fmt(arr),leave:fmt(arr+dwellOf(p)),_dur:dwellOf(p)})); t=arr+dwellOf(p); prevAddr=p.address; }
  }
  if(!lunchDone && result.length && ls>=s){
    let larr = lunchLoc ? t + smartTravel(prevAddr,lunchLoc) : Math.max(t,ls); larr=Math.max(larr,ls);
    result.push({lunch:true,arrive:fmt(larr),leave:fmt(larr+lunchDur),address:lunchLoc||''}); t=larr+lunchDur; if(lunchLoc) prevAddr=lunchLoc;
  }
  const homeMin = result.length ? t+smartTravel(prevAddr,endLoc) : s;
  return {result, home:fmt(homeMin), lateFix};
}

// 產生當日路線：自動收集（到期/分級）+ 加料(條件/指定) → 依急迫度挑入當日上限 → 套智慧時間排序
function planWeek(){
  syncWeek(); syncSmart(); syncRoute();
  const date=weekCfg.date||todayStr();
  const includeGrade=weekCfg.mode==='all';
  // 收集到期/分級對象，截止日＝當日（當日（含）以前到期者才納入）
  let targets=weekCfg.mode==='none'?[]:collectVisitTargets(weekCfg.regions,date,includeGrade);
  ...
  if((routeCfg.rules||[]).some(r=>r.status||r.channel||r.grade)){
    ...
    routeCfg.rules.forEach(r=>{ if(!(r.status||r.channel||r.grade))return;
      pool.filter(x=>!used.has(x.name+x.address)&&!seen.has(x.name+x.address)&&(!r.status||x.status===r.status)&&(!r.channel||x.channel===r.channel)&&(!r.grade||x.grade===r.grade)).sort(byDue).slice(0,r.n).forEach(x=>{ used.add(x.name+x.address); seen.add(x.name+x.address); targets.push(Object.assign({},x,{reason:'加料',sort:x.due?x.due.sort:998,overdue:x.due?x.due.sort<0:false})); }); });
  }
  // 加料：手動指定（smartCfg.picks）
  smartCfg.picks.forEach(p=>{
  ...
  if(!targets.length){ weekCfg.plan=null; weekCfg._last=`<div class="card empty"><div class="big">📭</div>${date} 沒有到期或指定的對象。<br>到「我的客戶／名單」設定分級或下次拜訪日，或放寬上方「客戶類型／區域／鄉鎮／通路／分級」篩選，或用「＋新增客戶」加入特定對象。</div>`; renderRoute(); return; }
  // 依鄉鎮急迫度排序：同鄉鎮集中、急的先排
  ...
  const cap=weekCfg.maxPerDay;
  const scheduled=targets.slice(0,cap), overflow=targets.slice(cap);
  const items=scheduled.slice();
  // 額外行程（固定時段約訪）：僅納入與當日相同日期者
  (weekCfg.extras||[]).forEach(x=>{
    if(x.date!==date) return;
    items.push({kind:'extra', id:x.id, name:x.name||'約訪', channel:'其他', grade:'', address:x.address||'', phone:'', district:district(x.address||''), reason:'約訪', sort:-2, overdue:false, fixed:x.time});
  });
  ...
  const buildBucket=()=>{ const sd=scheduleDayPlan(items, cfg); return {date, wd:new Date(date).getDay(), stops:sd.result, home:sd.home, lateFix:sd.lateFix}; };
  weekCfg.plan={date, cfg, overflow:overflow.map(o=>({name:o.name,district:o.district})), bucket:buildBucket(), overdue:targets.filter(t=>t.overdue).length, total:targets.length, scheduled:items.filter(t=>t.kind!=='extra').length};
  weekCfg._last='';
  renderRoute();
  setTimeout(()=>{ const el=$('#week-result'); if(el) el.scrollIntoView({behavior:'smooth',block:'start'}); },60);
  // 線上 OSRM 校正後重排一次（只送鄉鎮中心座標、不含個資）
  if(navigator.onLine!==false){
    const lls=[smartLatLng(cfg.startLoc),smartLatLng(cfg.endLoc),smartLatLng(cfg.lunchLoc)].concat(items.map(t=>smartLatLng(t.address))).filter(Boolean);
    osrmFillTable(lls).then(ok=>{ if(ok && weekCfg.plan){ weekCfg.plan.bucket=buildBucket(); weekCfg.plan.osrm=true; const el=$('#week-result'); if(el) el.innerHTML=renderPlanResult(); } }).catch(()=>{});
  }
}

function renderPlanResult(){
  const p=weekCfg.plan; if(!p) return weekCfg._last||'';
  const today=todayStr();
  const b=p.bucket; const cnt=b.stops.filter(s=>!s.lunch).length;
  let h=`<div class="sec-title"><span class="bar"></span>當日拜訪路線${p.osrm?' <span class="badge" style="background:#1b4d8f;color:#dceaff;float:right">✅ 線上路網校正</span>':''}</div><div class="card">`;
  h+=drow('日期', `${p.date}（週${WD_NAME[b.wd]}）${p.date===today?'　•今日':''}`);
  h+=drow('排入', `${p.scheduled} 家（逾期 ${p.overdue}）`);
  if(p.overflow.length) h+=drow('未排入', `${p.overflow.length} 家超過「最多家數」上限，可調高家數或改排其他日`);
  h+=`</div>`;
  h+=`<div class="sec-title" style="margin-top:14px"><span class="bar"></span>${p.date.slice(5)}（週${WD_NAME[b.wd]}）　(${cnt} 家)</div>`;
  h+= cnt ? dayStopsHTML(b, p.cfg, p.date===today) : `<div class="card empty" style="padding:16px">這天暫無安排</div>`;
  const saved=loadWeekPlan();
  h+=`<div class="btn-row" style="gap:8px;margin-top:12px"><button class="btn btn-pri" onclick="saveWeekPlan()">💾 寫入存檔（當天看今日行程）</button><button class="btn btn-out" onclick="planWeek()">🔄 重新規劃</button>${saved?`<button class="btn btn-gray" onclick="clearWeekPlan()">清除存檔</button>`:''}</div>`;
  if(saved) h+=`<div class="tagline" style="margin:2px 2px 0">已存檔於 ${(saved.savedAt||'').slice(0,16).replace('T',' ')}。可分別產生並存多天，到了那天頁首會自動顯示當日行程。</div>`;
  h+=`<div class="tagline" style="margin:8px 2px 0">排序原則：逾期/急迫優先、同鄉鎮集中減少往返；抵達時刻以最近鄰＋2-opt 就近順序＋約定到達＋中午休息估算。站間車程離線估算（鄉鎮中心點直線距離×1.3 倍、依距離抓 24–88 km/h），連網會用 OSRM 實際路網校正（只送鄉鎮中心座標、不含個資）。實際依路況而異，僅供參考。</div>`;
  return h;
}
...
function dayStopsHTML(b, cfg, isToday, hideFinish){
  ...
  let h='';
  if(cfg && cfg.endTime && b.home && toMin(b.home)>toMin(cfg.endTime)) h+=`<div class="info" style="background:var(--amber-l);color:var(--amber)">⚠️ 預估約 ${b.home} 才到家，略晚於 ${cfg.endTime}，可減少家數或提早出發。</div>`;
  h+=`<div class="card" style="padding:4px 14px">`;
  let idx=0;
  b.stops.forEach(x=>{
    if(x.lunch){ h+=`<div class="item" style="background:#fdf6e8"><div class="avatar" style="background:#c4811b">🍱</div><div class="body"><div style="display:flex;align-items:baseline;gap:8px;flex-wrap:wrap"><span style="font-size:20px;font-weight:800;line-height:1.05">${x.arrive}</span><span style="font-size:12px;color:var(--muted)">中午休息</span></div><div class="nm" style="margin-top:4px">🍱 中午休息</div>${x.address?`<div class="sub">${esc(x.address)}</div>`:''}</div><div class="meta">${x.address?`<a href="https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(x.address)}" target="_blank" onclick="event.stopPropagation()">導航</a>`:''}</div></div>`; return; }
    idx++;
...
function saveWeekPlan(){
  if(!weekCfg.plan){ toast('請先產生路線'); return; }
  const p=weekCfg.plan; const b=p.bucket;
  const slimB={date:b.date, wd:b.wd, home:b.home,
  ...
  if(!plan || !Array.isArray(plan.buckets)) plan={buckets:[]};
  plan.cfg=p.cfg;
  plan.buckets=(plan.buckets||[]).filter(x=>x.date!==b.date);
  plan.buckets.push(slimB);
  plan.buckets.sort((a,b)=>a.date.localeCompare(b.date));
  plan.start=plan.buckets[0].date; plan.weekEnd=plan.buckets[plan.buckets.length-1].date;
  plan.savedAt=new Date().toISOString();
  LS.set('crm_week_plan', plan);
  sfx('success');
  toast(`已寫入 ${b.date.slice(5)} 行程，當天打開頁首即見今日行程`);
  renderRoute();
}
...
function renderTodayCard(){
  ...
  if(!plan || !Array.isArray(plan.buckets) || !plan.buckets.length) return `<div class="info" style="background:#eef3fb;color:#143a6e">尚未寫入行程。請在下方選好「拜訪日期」、產生當日路線後按「💾 寫入存檔」，到了那天打開這頁，頁首就會直接顯示今日行程。</div>`;
  const b=plan.buckets.find(x=>x.date===today);
  if(!b){ const dates=plan.buckets.map(x=>x.date.slice(5)).join('、');
    return `<div class="card" style="padding:12px 14px"><div style="display:flex;align-items:center;gap:8px"><span style="font-size:13px;font-weight:700;color:#143a6e">📅 已存行程：${dates}</span><button class="rule-del" style="margin-left:auto" onclick="clearWeekPlan()">✕ 清除</button></div><div style="font-size:12.5px;color:var(--muted);margin-top:6px">今天沒有已存的行程，可於下方選日期重新規劃。</div></div>`; }
  const stops=b.stops.filter(s=>!s.lunch);
...
function weekFilterPool(){
  ...
  // 有地理篩選（區域或鄉鎮）時，目標客戶即使尚未設排程/分級也要列出；否則只列已設排程者，避免全部名單湧入
...
function syncSmart(){
  ...
  document.querySelectorAll('.pk-row').forEach(el=>{
...
// ========== 每週拜訪資料/設定（整合路線頁使用）==========
let weekCfg = { date:'', start:'', days:[1,2,3,4,5], maxPerDay:6, regions:[], mode:'all',
...
function thisMonday(){ const d=new Date(); const w=d.getDay(); d.setDate(d.getDate()+(w===0?-6:1-w)); return ymdLocal(d); }
function gradeFreq(g){ return GRADE_FREQ[g]||null; }
function syncWeek(){
  const s=$('#w-date'); if(s)weekCfg.date=s.value||weekCfg.date;
  const m=$('#w-max'); if(m)weekCfg.maxPerDay=Math.max(1,Math.min(12,+m.value||6));
  const dw=$('#w-dwell'); if(dw)weekCfg.dwell=Math.max(5,+dw.value||SMART_DWELL);
  // 區域為多選下拉，由 weekDDPick 直接維護 weekCfg.regions；排入對象 mode 由 weekDDSet 維護
}
...
function weekTownOptions(){
  ...
  SEED.forEach(p=>{ const ad=p.address||''; if(!ad)return; if(!(cores.some(core=>normR(ad).includes(core))||regs.some(rg=>normR(p.region||'')===normR(rg))))return; const d=district(ad); if(d)set.add(d); });
...
function syncRoute(){
  // 加料條件組合：只在加料展開時才有 DOM；缺欄位時不覆蓋既有設定
  if(!Array.isArray(routeCfg.rules)) routeCfg.rules=[];
  const nlEl=$('#r-nl'); if(nlEl) routeCfg.nl=nlEl.value;
  if($('#rules')) routeCfg.rules=readRules();
}
